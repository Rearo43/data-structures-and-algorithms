...
it("should return the first word eligible of a repeat in a string", () => {
  expect(repeatedWordFirstAppearance(strOne)).toStrictEqual(
    `Repeating Word: A, Length of String: 10`
  );
  expect(repeatedWordFirstAppearance(strTwo)).toStrictEqual(
    `Repeating Word: IT, Length of String: 120`
  );
  expect(repeatedWordFirstAppearance(strThree)).toStrictEqual(
    `Repeating Word: WAS, Length of String: 23`
  );
});

it("should return the first word eligible of a repeat in a string", () => {
  expect(repeatedWord(strOne)).toStrictEqual(
    `Repeating Word: A, Length of String: 10`
  );
  expect(repeatedWord(strTwo)).toStrictEqual(
    `Repeating Word: IT, Length of String: 120`
  );
  expect(repeatedWord(strThree)).toStrictEqual(
    `Repeating Word: SUMMER, Length of String: 23`
  );
});
